 ...
 if (joy != NULL) {
	...
	for (q=0; q<joy_buttons (joy); q++) {
		if (joy_getbutton (q, joy)) {
			if (q == 0)  { joyExKey |= GP2X_SPACE; }
			if (q == 1)  { joyExKey |= GP2X_LCTRL; }
			if (q == 2)  { joyExKey |= GP2X_LALT; }
			if (q == 3)  { joyExKey |= GP2X_T; }

			if (q == 4)  { joyExKey |= GP2X_TAB; }
			if (q == 5)  { joyExKey |= GP2X_RETURN; }
			if (q == 6)  { joyExKey |= GP2X_TAB; } /* left shoulder button 2 */
			if (q == 7)  { joyExKey |= GP2X_RETURN; } /* right shoulder button 2 */
			if (q == 8)  { joyExKey |= GP2X_5; }
			if (q == 9)  { joyExKey |= GP2X_1; }

			if (q == 10) { joyExKey |= GP2X_P; }
			if (q == 11) { joyExKey |= GP2X_P; }
		}
	}
	return joyExKey;
 } else {
